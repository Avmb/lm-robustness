...
parser.add_argument('--max_words_per_sent', type=int, default='100',
                    help='maximum number of words per sentence to generate')
...
def generate_sentences_from_prefixes(test_sentences, test_batch_size, args, average_ensemble):
    ...
    with open(args.outf, 'w') as outf:
        with torch.no_grad():
            for sent_id, sent in enumerate(test_sentences):
                ...
                test_loss, test_student_loss, hidden, log_prob = evaluate_and_compute(test_data, test_mask, test_batch_size, average_ensemble=average_ensemble, rnd_enable=args.rnd_enable)
                generate_sentence(hidden, log_prob, outf)
                print(len(sent), test_loss, math.exp(test_loss), test_student_loss)

def evaluate_and_compute(data_source, data_source_mask, batch_size=10, average_ensemble=True, rnd_enable=False):
    total_loss = 0
    ...
    with torch.no_grad():
        ...
        parallel_rv = parallel_model(*hidden, input=data, average_ensemble=average_ensemble, return_student_distill_loss=True, flatten_returned_lists=True, enable_rnd_tune=rnd_enable)
    ...
    return total_loss.item() / len(data_source), total_student_loss.item() / len(data_source), hidden, log_prob[-1]

def generate_sentence(hidden, output, outf):
    ninf = torch.scalar_tensor(-np.float('Inf'))
    input = Variable(torch.zeros(1, 1).long())
    if args.cuda:
        input.data = input.data.cuda()
    for i in range(args.max_words_per_sent):
...
# Run on test data.
generate_sentences_from_prefixes(corpus.test_sentences, test_batch_size, args, average_ensemble=not args.no_average_ensemble)
