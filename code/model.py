...
class RNNModel(nn.Module):
    ...
    def __init__(self, rnn_type, ntoken, ninp, nhid, nhidlast, nlayers, 
        ...
        self.rnd_pre_apply=False
        self.use_dropout = True
        self.state_post_proc_pre_update=False
        self.state_proc_only_on_last_layer=False
    ...
    def forward(self, *hidden, input=None, return_h=False, return_prob=False, 
        ...
        for l, rnn in enumerate(self.rnns):
            ...
            if (not self.state_proc_only_on_last_layer) or (l == len(self.rnns)-1):
                ...
                if self.rnd_pre_apply:
                    state_pre_proc = state_post_proc
                    state_post_proc = None

            current_input = raw_output
            if self.ndistilstudents  == 0:
                raw_output, new_h = rnn(current_input, hidden[l], 
                                        distill_loss_acc=distill_loss_acc, 
                                        state_pre_proc=state_pre_proc, state_post_proc=state_post_proc,
                                        state_post_proc_pre_update=self.state_post_proc_pre_update,
                                        state_proc_only_on_last_layer=self.state_proc_only_on_last_layer)
            else:
                raw_output, new_h = rnn(current_input, hidden[l], distill_loss_acc=distill_loss_acc, 
