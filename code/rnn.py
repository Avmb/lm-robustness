...
class RNN(nn.Module):
    ...
    def _forward_rnn(self, cell, x, h, L, state_pre_proc=None, state_post_proc=None, state_post_proc_pre_update=False, **kwargs):
        max_time = x.size(0)
        output = []
        ...
        for time in range(max_time):
            if state_pre_proc is not None:
                h = state_pre_proc(h, **kwargs)
            ...
            if self.c_on:
                ...
                if state_post_proc_pre_update and (state_post_proc is not None):
                    new_h = state_post_proc(new_h, **kwargs)
                h = new_h

            if (not state_post_proc_pre_update) and (state_post_proc is not None):
                new_h = state_post_proc(new_h, **kwargs)
            output.append(new_h)
    ...
    def forward(self, x, h=None, L=None, state_post_proc=None, 
                state_proc_only_on_last_layer=False, state_pre_proc=None, **kwargs):
        if self.batch_first:
            x = x.transpose(0, 1)
        ...
        for layer in range(self.num_layers):
            ...
            state_pre_proc_cur_layer = state_pre_proc if ((not state_proc_only_on_last_layer) or (layer == self.num_layers-1)) else None
            state_post_proc_cur_layer = state_post_proc if ((not state_proc_only_on_last_layer) or (layer == self.num_layers-1)) else None
            if layer == 0:
                layer_output, layer_state = self._forward_rnn(
                    cell, x, h_layer, L, state_pre_proc=state_pre_proc_cur_layer, state_post_proc=state_post_proc_cur_layer, **kwargs)
            else:
                layer_output, layer_state = self._forward_rnn(
                    cell, layer_output, h_layer, L, state_pre_proc=state_pre_proc_cur_layer, state_post_proc=state_post_proc_cur_layer, **kwargs)
